...
### Общие данные
- Название основного репозитория: `RPS`
- Описание проекта: `Учебная практика по теме программирования`

### Данные участников
**Тимлид:**
- Логин на GitHub: `Fram1se`
- Полный путь к локальной папке проекта: `https://github.com/Fram1se/RPS`

**Падаван:**
- Логин на GitHub: `Art-021`
- Полный путь к локальной папке проекта: `https://github.com/Art-021/RPS.git`
...
**Штатный результат:** Создан основной репозиторий с README.md по адресу: `https://github.com/Fram1se/RPS`
...
- **Действие 2.1:** Перейти к основному репозиторию
  - Команда: `Переход по ссылке: https://github.com/Fram1se/RPS`
...
**Штатный результат:** Создан форк по адресу: `https://github.com/Art-021/RPS` с пометкой "forked from Fram1se/RPS"
...
- **Действие 3.3 (Тимлид):** Клонировать основной репозиторий
  - Команда: `git clone https://github.com/Art-021/RPS.git`

#### Действия для Падавана
**Действия:**
- **Действие 3.4 (Падаван):** Перейти в рабочую директорию
  - Команда: `cd C:/Workspace/Fram1se/RPS-1`
...
- **Действие 4.3:** Добавить связь с основным репозиторием
  - Команда: `git remote add upstream https://github.com/Fram1se/RPS.git`
...
- **Действие 6.2:** Перейти на GitHub форка падавана
  - Команда: `Открыть браузер → https://github.com/Art-021/RPS`
...
- **Основной репозиторий:** `https://github.com/Fram1se/RPS`
- **Форк падавана:** `https://github.com/Art-021/RPS`
- **Pull Request:** `https://github.com/Art-021/RPS/pull/Fram1se`
...
- [x] Настроена видимость (Public/Private)
- [x] Проверена доступность репозитория

**Команды выполнения:**
```bash
# Локальная настройка
git config --global user.name "Fram1se"
git config --global user.email "malexd.888@gmail.com"

# Клонирование основного репозитория
git clone https://github.com/Fram1se/RPS.git
cd C:/workspace/shinkurka/RPS-1
```

**Результаты:**
- URL основного репозитория: `https://github.com/Fram1se/RPS`
- Количество начальных файлов: `3`
...
|---|---------|-------|--------|---------|
| 1 | `Fram1se` | `RPS` | `[ ] Открыт [x] Принят [ ] Отклонен` | `https://github.com/Fram1se/RPS/branches` |

**Проверка изменений:**
- [x] Код соответствует требованиям
- [x] Нет конфликтов слияния
- [x] Коммиты имеют понятные сообщения
- [x] Изменения логичны и завершены
...
- Количество файлов: `3`
- Количество коммитов: `32`
- Актуальные ветки: `RPS`
...
**Выполненные действия:**
- [x] Создан форк основного репозитория
- [x] Локально клонирован форк
- [x] Настроена связь upstream

**Команды выполнения:**
```bash
# Клонирование форка
git clone https://github.com/Art-021/RPS.git
cd C:/workspace/Fram1se/RPS-1
...
- Имя ветки: `main`
- Цель изменений: `Работа с коммитами`
...
**Внесенные изменения:**
- Созданные файлы: `README.md; RPS.py; .gitignor`
- Измененные файлы: `README.md; RPS.py`
- Удаленные файлы: `-`
...
|----------|---------------------|-----------------|---------------------|
| Тимлид | `26` | `RPS.py; README.md; .gitignore` | `внесение коммитов` |
| Падаван | `16` | `0` | `внесение коммитов` |
...
### Возникшие трудности:
1. **Проблема:** `Проблема в совместной разработке`
   - **Решение:** `создание форка и применения коммитов`
   - **Результат:** `[x] Решено [ ] Частично решено [ ] Не решено`

2. **Проблема:** `понимание как работает git`
   - **Решение:** `получение информации через deepseek`
   - **Результат:** `[ ] Решено [x] Частично решено [ ] Не решено`

### Типовые ошибки и их исправление:
- **Ошибка:** Конфликт слияния
  - **Как исправили:** `правильно настроить upstream`
- **Ошибка:** Неправильная настройка remote
  - **Как исправили:** `провод диагностики и правильная ссылка`
...
### Тест для Тимлида:
- [x] `git status` показывает "working tree clean"
- [ ] `git log` отображает мерж-коммит от PR
- [x] Все ветки актуальны (`git fetch --all`)
- [x] На GitHub отображаются все изменения

### Тест для Падавана:
- [x] `git remote -v` показывает origin и upstream
- [x] Локальная main ветка синхронизирована с upstream
- [ ] PR имеет статус "Merged"
- [x] Локально можно переключиться на обновленную main
...
### Что получилось:
- [x] Организована командная работа
- [ ] Освоен процесс Fork-Pull Request
- [ ] Выполнена поставленная задача
- [x] Код успешно объединен

### Что нужно улучшить:
- `________________`
- `________________`

### Общая оценка работы команды: `[ ] Отлично [x] Хорошо [ ] Удовлетворительно`
...
## Описание задания
[работа в команде с помощью Git]
